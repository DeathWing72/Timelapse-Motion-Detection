...
class Window 
{
    ...
    {
        final double[] lumaValues = new double[getWindowSize()* getWindowSize()];
        int counter = 0;
        for (int i = getMinX(); i < getMaxX(); i++) 
        {
            for (int j = getMinY(); j < getMaxY(); j++) 
            {
                ...
                final double Y = ((double) red * RED_COEFFICIENT) +  ((double) green * GREEN_COEFFICIENT) + ((double) blue * BLUE_COEFFICIENT);
